...
export function validateAnalyzeBody(body) {
  ...
  if (trimmed.length > 500) {
    return { error: "Input must be 500 characters or less", status: 400 };
  }
...
/**
 * Validate checkout POST body. Only priceId, successUrl, cancelUrl allowed.
 */
export function validateCheckoutBody(body) {
  ...
  return { priceId: priceId.trim(), successUrl: successUrl.trim(), cancelUrl: cancelUrl.trim() };
}
