...
function Stat({label,value,color}){return(<div style={{padding:16,background:PN+"88",borderRadius:CARD_RADIUS,border:"none"}}><div style={{fontSize:10,color:DM,fontWeight:800,letterSpacing:0.5,marginBottom:6,textTransform:"uppercase"}}>{label}</div><div style={{fontSize:18,fontWeight:700,color:color||TX,lineHeight:1.3}}>{value||"N/A"}</div></div>);}
function Head({title}){return(<div style={{marginBottom:10,marginTop:18}}><span style={{fontSize:11,fontWeight:800,letterSpacing:1.2,color:DM,textTransform:"uppercase"}}>{title}</span></div>);}

function BentoCard({title,children,span}){return(<div style={{gridColumn:span||"span 1",background:CD,borderRadius:CARD_RADIUS,padding:0,boxShadow:"0 4px 24px rgba(0,0,0,0.4)",border:"none",overflow:"hidden"}}>{title&&<div style={{display:"flex",alignItems:"center",padding:"14px 20px",borderBottom:"1px solid "+BD+"60"}}><span style={{fontSize:11,fontWeight:800,letterSpacing:1.5,color:TX,textTransform:"uppercase"}}>{title}</span></div>}<div style={{padding:title?"20px":"24px"}}>{children}</div></div>);}
...
function PieChart({competitors,compact,large}){
  ...
  for(var i=0;i<list.length;i++){
    ...
    if(pct>0){slices.push({name:list[i].name,value:pct,color:PIE_COLORS[i%PIE_COLORS.length],source:list[i].shareSource||null});knownTotal+=pct;}
  }
  ...
  return(
    <div style={wrapStyle}>
      <svg width={sz} height={sz} viewBox="0 0 180 180">
        {paths.map(function(p,k){return <path key={k} d={p.d} fill={p.color} stroke={BG} strokeWidth={1.5} style={{cursor:p.source?"pointer":"default",transition:"opacity 0.15s"}} onMouseEnter={function(e){if(p.source)e.currentTarget.style.opacity="0.75";}} onMouseLeave={function(e){e.currentTarget.style.opacity="1";}} onClick={function(){if(p.source)window.open(p.source,"_blank");}}/>;})
        }
        <circle cx={cx} cy={cy} r={24} fill={CD} stroke={GRIDLINE} strokeWidth={1}/>
      </svg>
      <div style={{display:"flex",flexDirection:"column",gap:4}}>
        {slices.map(function(s,k){var hasSource=!!s.source;return(
          <div key={k} style={{display:"flex",alignItems:"center",gap:6,cursor:hasSource?"pointer":"default",padding:"2px 4px",borderRadius:2,transition:"background 0.15s"}} onMouseEnter={function(e){if(hasSource)e.currentTarget.style.background=BD+"44";}} onMouseLeave={function(e){e.currentTarget.style.background="transparent";}} onClick={function(){if(hasSource)window.open(s.source,"_blank");}}>
            <div style={{width:8,height:8,borderRadius:1,background:s.color,flexShrink:0}}/>
            <span style={{fontSize:11,color:s.name==="Unknown"?DM:TX}}>{s.name}</span>
            <span style={{fontSize:11,fontWeight:600,color:s.name==="Unknown"?DM:CHART_COLOR}}>{s.value+"%"}</span>
            {hasSource&&<svg width={8} height={8} viewBox="0 0 12 12" fill="none"><path d="M3.5 1.5h7m0 0v7m0-7L3 9" stroke={s.color} strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round"/></svg>}
          </div>
        );})}
      </div>
    </div>
...
function MarketChart5Y({history5y,large,compact}){
  ...
  var w=large?480:320;var h=large?200:140;var pad={l:40,r:20,t:24,b:32};
...
function CompetitorChart5Y({competitors5y,large,compact}){
  ...
  var gridLines=[];for(var g=1;g<=4;g++){var gy=pad.t+chartH*(1-g/4);gridLines.push(<line key={g} x1={pad.l} y1={gy} x2={w-pad.r} y2={gy} stroke={GRIDLINE} strokeWidth={1} strokeDasharray="2,2" opacity={0.6}/>);}
  var wrapStyle=compact?{marginBottom:0}:{padding:16,background:CD,borderRadius:CARD_RADIUS,marginBottom:16,border:"1px solid "+BD+"30"};
  return(<div style={wrapStyle}>{!compact&&<div style={{fontSize:10,fontWeight:800,letterSpacing:1,color:DM,marginBottom:12,textTransform:"uppercase"}}>Amount of competitors</div>}<ChartTooltip visible={tooltip.visible} x={tooltip.x} y={tooltip.y} title={tooltip.year?"Year "+tooltip.year:""} value={tooltip.value} unit="competitors"/><svg width={w} height={h} viewBox={"0 0 "+w+" "+h} style={{display:"block"}}><g>{gridLines}</g><path d={pathD} fill="none" stroke={CHART_COLOR} strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round"/>{pts.map(function(p,i){return(<g key={i} onMouseEnter={function(e){setTooltip({visible:true,x:e.clientX,y:e.clientY,year:data[i].year||"",value:String(p.v)});}} onMouseLeave={function(){setTooltip({visible:false});}}><circle cx={p.x} cy={p.y} r={4} fill={CHART_COLOR} style={{cursor:"pointer"}}/><text x={p.x} y={h-8} textAnchor="middle" fill={DM} style={{fontSize:10,fontWeight:600}}>{data[i].year||""}</text></g>);})}</svg></div>);
}
...
function SpiderChart({competitors,compact,large}){
  ...
  var totalSize=large?320:260;
  var padding=52;
  var cx=totalSize/2;var cy=totalSize/2;
  var R=Math.min(totalSize/2-padding,cx-padding-24);
  var n=5;
  var angle0=-90;
...
function DeepResearchToggle({deep,setDeep,canDeep,deepUsed,deepLimit,colors}){
...
export default function App(){
  ...
  var canRunLive=!!(user&&(tier==="pro"||tier==="business"||admin));
  ...
  useEffect(function(){if(!user||!canDeep)return;var m=new Date().toISOString().slice(0,7);supabase.from("usage").select("deep_count").eq("user_id",user.id).eq("month",m).maybeSingle().then(function(r){setUsage(r.data);}).catch(function(){});},[user,canDeep]);
  ...
  function doRun(){
    ...
    .then(function(res){if(res.error)throw new Error(res.error);setData(res);if(deep&&canDeep&&user){var m=new Date().toISOString().slice(0,7);supabase.from("usage").select("deep_count").eq("user_id",user.id).eq("month",m).maybeSingle().then(function(r){setUsage(r.data);});}})
  ...
  function run(){
    if(!idea.trim()||loading)return;
    if(deep&&!canDeep)return;
    if(!canRunLive){router.push("/pricing");return;}
    var t=idea.trim();
    if(t.length<10){setErr("Please describe your business idea in a few words (e.g. 'Meal kits in Denmark')");return;}
    if(t.split(/\s+/).filter(Boolean).length<2){setErr("Please describe your idea with at least 2 words (e.g. 'Dog food EU')");return;}
  ...
  return(
    <>
      <div style={{padding:"20px 24px 16px",borderBottom:"1px solid "+BD+"40"}}>
        <h1 style={{fontSize:28,fontWeight:800,letterSpacing:1,color:TX,textTransform:"uppercase",marginBottom:16}}>{data&&sm.title?sm.title:"Market Research"}</h1>
        <div style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
          <div style={{flex:1,minWidth:220,position:"relative"}}>
            <input ref={ref} value={idea} onChange={function(e){var v=e.target.value;if(v.length<=500)setIdea(v);}} onKeyDown={function(e){if(e.key==="Enter")run()}} placeholder="Idea: e.g. Meal-kit subscription, Denmark" disabled={loading} maxLength={500} style={{width:"100%",padding:"12px 40px 12px 16px",background:CD,border:"1px solid "+BD,borderRadius:BTN_RADIUS,color:TX,fontSize:13,boxSizing:"border-box",outline:"none"}}/>
            <span style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",fontSize:11,color:DM}}>{idea.length}/500</span>
          </div>
          <DeepResearchToggle deep={deep} setDeep={setDeep} canDeep={canDeep} deepUsed={deepUsed} deepLimit={deepLimit} colors={{DM,AC,BD,CD,TX,YL}}/>
          <button onClick={run} disabled={loading||!idea.trim()||(deep&&!canDeep)||!canRunLive} title={!canRunLive?"Sign in and upgrade to Pro for live research":""} style={{padding:"12px 22px",background:loading?BD:canRunLive?AC:BD,color:loading?DM:canRunLive?BG:DM,border:"none",borderRadius:BTN_RADIUS,fontWeight:700,fontSize:12,cursor:loading||!canRunLive?"wait":"pointer",opacity:!idea.trim()?0.5:1}}>{loading?(deep?"Deep research...":"Analyzing..."):"Run research"}</button>
        </div>
      </div>
      {ideaConfirmOpen&&(
      ...
      {loading&&<div style={{padding:40,textAlign:"center"}}><div style={{maxWidth:320,margin:"0 auto 16px"}}><div style={{height:4,background:BD,borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:"40%",background:AC,borderRadius:4,animation:"lume-progress 1.5s ease-in-out infinite"}}/></div></div><div style={{fontSize:13,fontWeight:600,color:AC}}>Researching...</div></div>}
      {err&&<div style={{margin:20,padding:18,background:RD+"15",border:"1px solid "+RD+"44",borderRadius:BTN_RADIUS,fontSize:12,color:RD}}>{err}</div>}
      {data&&!loading&&<AnalysisResultView data={data} canExport={tier==="pro"||tier==="business"} isSampleData={isSampleData}/>}
      {!data&&!loading&&!err&&(<div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:32}}><div style={{width:"100%",maxWidth:540,background:CD,borderRadius:CARD_RADIUS,padding:36,boxShadow:"0 8px 32px rgba(0,0,0,0.4)"}}><div style={{display:"flex",alignItems:"center",gap:14,marginBottom:24}}><LumeLogo size={44}/><div><div style={{fontWeight:800,fontSize:22,letterSpacing:0.5,color:TX,textTransform:"uppercase"}}>Lume</div><div style={{fontSize:12,color:DM,marginTop:4}}>Market research engine</div></div></div><div style={{fontSize:14,color:MU,lineHeight:1.6,marginBottom:24}}>Every idea deserves to be understood before it's built.</div><div style={{display:"flex",gap:10,flexWrap:"wrap",justifyContent:"center"}}>{["Meal kits Denmark","Dog food EU","SaaS accounting"].map(function(ex,i){return(<button key={i} onClick={function(){setIdea(ex);tryWithSample(ex);}} style={{padding:"10px 18px",background:PN,border:"1px solid "+BD,borderRadius:BTN_RADIUS,fontSize:12,fontWeight:600,color:MU,cursor:"pointer",transition:"background 0.15s"}}>{ex}</button>);})}</div><div style={{marginTop:20,fontSize:12,color:DM,textAlign:"center"}}>Explore sample data, or sign in and upgrade to Pro for live research.</div></div></div>)}
    </>
  );
}
...
function TabBar({active,onChange}){
  return(<div style={{display:"flex",gap:4,padding:"12px 24px",borderBottom:"1px solid "+BD+"40",flexWrap:"wrap"}}>
    {TABS.map(function(t){var isActive=active===t;return(<button key={t} type="button" onClick={function(){onChange(t);}} style={{padding:"8px 16px",fontSize:12,fontWeight:600,color:isActive?TX:MU,background:isActive?CD:"transparent",border:"none",borderRadius:BTN_RADIUS,cursor:"pointer",textTransform:"capitalize"}}>{t}</button>);})}
  </div>);
}

export function AnalysisResultView({data,canExport,isSampleData}){
      ...
      <BentoCard title="Summary" span="span 1">
        <div style={{display:"flex",alignItems:"flex-start",gap:20,flexWrap:"wrap"}}>
          <div style={{flex:1,minWidth:200}}>
            <div style={{fontWeight:800,fontSize:20,letterSpacing:"-0.02em"}}>{sm.title||"Results"}</div>
            <div style={{fontSize:13,color:MU,marginTop:4,lineHeight:1.5}}>{sm.oneLiner}</div>
          </div>
          {isSampleData&&<Badge color={CN}>Sample data</Badge>}
          {sm.verdict&&<Badge color={vc[sm.verdict]||MU}>{sm.verdict}</Badge>}
          {sm.confidence&&<div style={{textAlign:"center"}}><div style={{fontFamily:"monospace",fontSize:28,fontWeight:800,color:sm.confidence>60?GR:YL}}>{sm.confidence+"%"}</div><div style={{fontSize:9,fontFamily:"monospace",color:DM}}>CONFIDENCE</div></div>}
          {canExport&&<button onClick={function(){var t=(sm.title||"lume-analysis").replace(new RegExp("[^a-z0-9]","gi"),"-");var blob=exportAnalysisToPdf(data);var a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=t+".pdf";a.click();URL.revokeObjectURL(a.href);}} style={{padding:"8px 14px",background:BD,border:"1px solid "+BD,borderRadius:6,fontSize:11,fontFamily:"monospace",fontWeight:700,color:MU,cursor:"pointer"}}>Export PDF</button>}
        </div>
      </BentoCard>
    </div>
