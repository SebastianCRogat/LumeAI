...
function Stat({label,value,color,expandable,maxLength}){return(<div style={{padding:16,background:PN+"88",borderRadius:CARD_RADIUS,border:"none"}}><div style={{fontSize:10,color:DM,fontWeight:800,letterSpacing:0.5,marginBottom:6,textTransform:"uppercase"}}>{label}</div>{expandable?<ExpandableText text={value||"N/A"} maxLength={maxLength||60} style={{fontSize:18,fontWeight:700,color:color||TX,lineHeight:1.3,display:"block"}}/>:<div style={{fontSize:18,fontWeight:700,color:color||TX,lineHeight:1.3}}>{value||"N/A"}</div>}</div>);}
    ...
    <div style={wrapStyle}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:6}}>
        {slices.map(function(s,k){var hasLink=!!s.source;var hasNote=!!s.sourceText;return(
          <div key={k}>
            <div style={{display:"flex",alignItems:"center",gap:6,cursor:hasLink?"pointer":"default",padding:"2px 4px",borderRadius:2,transition:"background 0.15s"}} onMouseEnter={function(e){if(hasLink)e.currentTarget.style.background=BD+"44";}} onMouseLeave={function(e){e.currentTarget.style.background="transparent";}} onClick={function(){if(hasLink)window.open(s.source,"_blank");}}>
              ...
              {hasLink&&<svg width={8} height={8} viewBox="0 0 12 12" fill="none"><path d="M3.5 1.5h7m0 0v7m0-7L3 9" stroke={s.color} strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round"/></svg>}
            </div>
            {hasLink&&<div style={{paddingLeft:18,fontSize:9,color:BL,cursor:"pointer",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:200}} onClick={function(){window.open(s.source,"_blank");}}>{s.source.replace(/^https?:\/\/(www\.)?/,"").split("/")[0]}</div>}
            {hasNote&&<div style={{paddingLeft:18,fontSize:9,color:DM,fontStyle:"italic",maxWidth:200,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={s.sourceText}>{s.sourceText}</div>}
          </div>
        );})}
      </div>
    </div>
...
function CompetitorCard({x,tc}){
  ...
  return(
    <div style={{background:"transparent",borderRadius:6,padding:14,marginBottom:10,borderLeft:"3px solid "+(tc[x.threat]||MU),borderBottom:"1px solid "+BD+"40"}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
        <span style={{fontWeight:700,fontSize:14}}>{x.name}</span>
        <Badge color={tc[x.threat]||MU}>{x.threat}</Badge>
      </div>
      <div style={{fontSize:11,color:DM,marginBottom:6,lineHeight:1.5}}>{x.desc}</div>
      {hasSW&&(
        showSW
...
function RunResearchButton({run,loading,idea,deep,canDeep,canRunLive,colors}){
  ...
  return(
    <span style={{position:"relative",display:"inline-flex"}} onMouseEnter={function(){setHover(true);}} onMouseLeave={function(){setHover(false);}}>
      <button onClick={run} disabled={loading||!idea.trim()||(deep&&!canDeep)||!canRunLive} style={{padding:"12px 22px",background:loading?BD:canRunLive?AC:BD,color:loading?DM:canRunLive?BG:DM,border:"none",borderRadius:BTN_RADIUS,fontWeight:700,fontSize:12,cursor:loading?"wait":!canRunLive?"default":"pointer",opacity:!idea.trim()?0.5:1}}>{loading?(deep?"Deep research...":"Analyzing..."):"Run research"}</button>
      {showTooltip&&(
        <div style={{position:"absolute",top:"calc(100% + 10px)",left:"50%",transform:"translateX(-50%)",padding:"14px 18px",background:CD,border:"1px solid "+BD,borderRadius:6,fontSize:12,color:TX,lineHeight:1.5,width:280,zIndex:9999,boxShadow:"0 8px 24px rgba(0,0,0,0.5)",pointerEvents:"none",animation:"lume-fade-in 0.2s ease"}}>
          <div style={{position:"absolute",top:-5,left:"50%",transform:"translateX(-50%)",width:0,height:0,borderLeft:"6px solid transparent",borderRight:"6px solid transparent",borderBottom:"6px solid "+BD}}/>
          <div style={{position:"absolute",top:-4,left:"50%",transform:"translateX(-50%)",width:0,height:0,borderLeft:"6px solid transparent",borderRight:"6px solid transparent",borderBottom:"6px solid "+CD}}/>
    ...
    <>
      <div style={{padding:"20px 24px 16px",borderBottom:"1px solid "+BD+"40"}}>
        ...
        <div style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <RunResearchButton run={run} loading={loading} idea={idea} deep={deep} canDeep={canDeep} canRunLive={canRunLive} colors={{BG,BD,AC,CD,TX,DM,BTN_RADIUS}}/>
        </div>
      </div>
      {ideaConfirmOpen&&(
