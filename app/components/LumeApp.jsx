...
export default function App(){
  ...
  var canRunLive=!!(user&&(tier==="pro"||tier==="starterkit"||tier==="business"||tier==="free"||admin));
  var canDeep=tier==="pro"||tier==="starterkit"||tier==="business"||admin;
  var deepLimit=admin?5:tier==="pro"||tier==="starterkit"?1:tier==="business"?5:0;
  var deepUsed=usage?.deep_count||0;
  var standardUsed=usage?.standard_count||0;
  var standardLimit=admin?50:tier==="business"?50:tier==="pro"?10:tier==="starterkit"?5:1;
      ...
      {hasResults?(
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:isMobile?"70vh":"75vh",padding:isMobile?"20px 10px":"32px 24px",width:"100%",minWidth:0,boxSizing:"border-box"}}>
          ...
          <div style={{marginTop:28,display:"flex",gap:10,flexWrap:"wrap",justifyContent:"center"}}>
            {["Meal kits Denmark","Dog food EU","SaaS accounting"].map(function(ex,i){return(<button key={i} onClick={function(){setIdea(ex);tryWithSample(ex);}} style={{padding:isMobile?"12px 18px":"10px 18px",minHeight:isMobile?44:undefined,background:CD,border:"1px solid "+BD,borderRadius:BTN_RADIUS,fontSize:12,fontWeight:600,color:MU,cursor:"pointer",transition:"all 0.15s"}}>{ex}</button>);})}
          </div>
          {!user&&<div style={{marginTop:isMobile?24:32,width:"100%",maxWidth:380,marginLeft:"auto",marginRight:"auto",padding:isMobile?"0 8px":0}}><button type="button" onClick={function(){router.push("/?signup=1");}} style={{width:"100%",display:"flex",flexDirection:isMobile?"column":"row",alignItems:"center",gap:isMobile?14:20,padding:isMobile?"16px 18px":"20px 24px",background:"transparent",borderRadius:16,border:"1px solid "+BD+"50",boxSizing:"border-box",textAlign:isMobile?"center":"left",cursor:"pointer",transition:"background 0.2s,border-color 0.2s"}} onMouseEnter={function(e){e.currentTarget.style.background=CD+"40";e.currentTarget.style.borderColor=BD+"99";}} onMouseLeave={function(e){e.currentTarget.style.background="transparent";e.currentTarget.style.borderColor=BD+"50";}}><div style={{flexShrink:0,width:isMobile?44:52,height:isMobile?44:52,borderRadius:14,background:AC+"12",display:"flex",alignItems:"center",justifyContent:"center",border:"1px solid "+AC+"28"}}><svg width={isMobile?24:28} height={isMobile?24:28} viewBox="0 0 32 32" fill="none"><rect x="6" y="14" width="20" height="12" rx="1.5" stroke={AC} strokeWidth="1.5" fill="none" opacity="0.9"/><path d="M16 14v12M6 20h20" stroke={AC} strokeWidth="1.5" opacity="0.8"/><path d="M16 14c0-2.5 1.8-5 4.5-5 1.5 0 3 .8 3 3s-1.5 3-3 3H16" stroke={AC} strokeWidth="1.4" strokeLinecap="round" fill="none" opacity="0.85"/><path d="M16 14c0-2.5-1.8-5-4.5-5-1.5 0-3 .8-3 3s1.5 3 3 3H16" stroke={AC} strokeWidth="1.4" strokeLinecap="round" fill="none" opacity="0.85"/><circle cx="16" cy="10" r="1.5" fill={AC} opacity="0.6"/></svg></div><div style={{flex:1,minWidth:0}}><div className="lume-gradient-shimmer" style={{fontSize:isMobile?13:14,fontWeight:600,letterSpacing:"-0.01em"}}>1 free research when you sign up</div><div style={{fontSize:isMobile?11:12,color:MU,lineHeight:1.45,marginTop:4}}>Create an account or try the examples above.</div></div></button></div>}
